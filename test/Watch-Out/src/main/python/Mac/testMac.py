...
OWN_bullets, ENEMY_bullets=NullType, NullType
CANFIRE,FIRED,DIE,isMENU,LOSE,BESTSCORE=False,False,False,True,NullType,0.0
PG_HP=5
...
def play(): 
    ...
    while(DIE==True):
        time.sleep(0.2)
    CANFIRE, FIRED, LOSE=False,False,NullType
    ENEMYT.start()
    timer.start()
    if(ENEMY_NUMBER=="0"):
        changeLevel()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if EXIT.checkForInput(MENU_MOUSE_POS):
                    if not CANFIRE:
                        ...
                        isMENU=True
                        main_menu()
                if event.button == 1 and OWN_bullets==NullType:
